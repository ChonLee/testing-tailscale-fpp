...
// CRITICAL: Prevent execution during FPP's plugin scanning
// Only run if explicitly called via plugin.php with nopage=1
if (!defined('FPPD') && 
    (empty($_SERVER['SCRIPT_NAME']) || 
     basename($_SERVER['SCRIPT_NAME']) !== 'plugin.php' ||
     empty($_GET['page']) ||
     $_GET['page'] !== 'api.php')) {
    // Not being called properly, exit silently
    exit;
}

// Additional check: Must have action parameter
